...
The proxy endpoints provide direct pass-through access to MCP and A2A servers registered in ARK. This enables tools like MCP inspector or A2A inspector to connect through the ark-api without needing direct access to the server ports.
...
Proxy requests to A2A servers:

```bash
# Proxy to A2A server root
curl http://localhost:8000/proxy/a2a/my-a2a-server
...
- **HTTP Methods**: Supports GET, POST, PUT, DELETE, PATCH, OPTIONS
- **Header Forwarding**: Automatically forwards request headers (except host/connection headers)
- **Server Headers**: Includes headers configured in the server spec
- **Query Parameters**: Preserves query parameters from the original request
- **Request Bodies**: Handles request bodies for POST/PUT/PATCH requests

### Error Responses

- `404` - Server not found
- `503` - Server has no resolved address (not ready yet)
- `502` - Proxy request failed (target server unreachable)

The proxy automatically resolves server addresses from Kubernetes CRD status fields (`resolvedAddress` for MCP servers, `lastResolvedAddress` for A2A servers) and includes any headers configured in the server spec.
...
<Tabs items={['Non-streaming', 'Streaming']}>
  <Tabs.Tab>
    ...
    context_id = response.ark['completedQuery']['status']['a2a']['contextId']
    ```
  </Tabs.Tab>
  <Tabs.Tab>
    ```python
    ...
    for chunk in stream:
        ...
        # Final chunk contains complete query object
        if hasattr(chunk, 'ark') and chunk.ark.get('completedQuery'):
            context_id = chunk.ark['completedQuery']['status']['a2a']['contextId']
    ```
  </Tabs.Tab>
</Tabs>
